...
<pre class="sh_ruby"><code>p ErbParser.new.erb_with_tags.parse("<%= name %>")</code></pre> while <pre class="sh_ruby"><code>p ErbParser.new.erb_with_tags.parse("Hello, <%= name %>!")</code></pre> would fail since the @erb_with_tags@ rule just covers text which starts with @<%=@ and ends with @%>@.
