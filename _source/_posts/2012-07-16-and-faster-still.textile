...
h2. Racer (with Sinatra)

"Racer":http://github.com/charliesome/racer by "Charlie Somerville":http://twitter.com/charliesome is a "Rack compliant Ruby web server". It is mainly based on "libuv":http://github.com/joyent/libuv. According to its "README":http://github.com/charliesome/racer/blob/master/README.md (worth a look just for the image ;) ), it is twice as fast as thin using a "Hello world!" app.
...
Now, why don't we get double the speed as with thin, as shown on "Racer's webpage":https://github.com/charliesome/racer, but just 10%? The thing is, instead of just returning "hello world", Picky needs to do a bit of work.
...
| 3: | T(thin) + T(picky) == 1.1*T(racer) + 1.1*T(picky) | from 2. |
| 4: | T(thin) - 1.1*T(racer) == 0.1*T(picky) | from 3. |
| 5: | T(thin) == 2*T(racer) | from 1. |
| 6: | 0.9*T(racer) == 0.1*T(picky) | from 4, 5. |
...
As Racer cannot yet accept on file descriptors, I am going to use http load balancers "Pen":http://siag.nu/pen/ and "Nginx":http://www.nginx.org/ and see how they fare on my 2 core MBP.
