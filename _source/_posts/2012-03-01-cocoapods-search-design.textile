...
Since I think the "rubygems search":http://rubygems.org/search is too slow, and not very impressive, I tried to make the "CocoaPods search":http://cocoapods.org an example of how such a search should be designed.
...
Many ideas come from the old gem search alternative, but a few features are new, compiled in the…

h2. Highlights

* "Automagic index updates via Github post receive hooks":#hooks
* "Making composite names (e.g. BlocksKit) searchable":#composites
* "Filtering by OS":#callbacks
* "Removing duplicates from results":#duplicates
...
Like Ruby gems, pods can run on multiple OS'. Either on iOS or on OS X.
...
This is problematic since it uses a lot of space, and also confuses the user.

We would like to make this invisible. Picky helps us by offering multiple JS callbacks. First of all we define a regexp to remove the platform search term:

<pre class="javascript"><code>var platformRemoverRegexp = /(platform|on\:\w+\s?)+/;</code></pre>

If you copy a search link like "http://cocoapods.org/?q=on:osx%20Kiwi":http://cocoapods.org/?q=on:osx%20Kiwi, Picky runs the @beforeInsert@ callback, while also checking the correct OS (not pictured), simply removing the OS part from the query:
...
Why are there duplicates in Picky's search results anyway? Picky returns categorized search results. For example, it might deem the combination of categories @"first_name", "last_name"@ more important, before all search results found in the categories @"street", "last_name"@. But this also means that the same entry can be contained in both combinations of categories!
...
Since cocoapods.org only wants to show an uncategorized list of result pods, we wish to remove duplicates.

We achieve this by using Picky's JS @success@ callback:
...
This goes through all combinations of categories (aka _allocations_) and removes entries if we've already seen them previously. This ensures we only see unique results.
...
* Enter "Luke 1.0":http://cocoapods.org/?q=luke%21.0 to get all pods written by a luke with version 1.0*.
